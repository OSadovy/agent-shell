...
Steering is always explicit.  =M-x agent-shell-prompt-queue= queues, as it always has.  =M-x agent-shell-prompt-steer= steers, and errors when there is no running turn to steer or the agent cannot steer.  When the agent declines a steer, the running turn is interrupted and the shell says so, rather than letting the agent carry on in a direction you believe you already corrected.
...
Steering is not additive.  An agent that interrupts may drop the instruction it was working on, so a steer can replace what the agent was already doing rather than adding to it.  Queue instead when the agent should finish first.
...
|                 | agent-shell-prompt-steer                                | Steer PROMPT into the turn the agent is currently running.                    |
